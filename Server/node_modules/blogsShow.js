...
exports.blogsShow = function(req, res){
    ...
    //记录loginaccount
    params.login.account = queryParam.loginaccount*1;
    ...
    if(queryParam.blogid != null){
        ...
        for(var i in blogs.blogs){
            if(blogs.blogs[i].blogId == blogId){
                blogMark = true;
                blogIndex = i;
                params.blog.blogId = blogs.blogs[i].blogId;
        ...
        for(var i in users.users){
            if(users.users[i].account == queryParam.account*1){
                userIndex = true;
        ...
        for(var j in blogs.blogs){
            if(blogs.blogs[j].belong == queryParam.account*1){
        ...
        console.log("personShow has already returned!");
        //console.log(params.comment.length);
        res.render(BASE_DIR + '/static/template/personShow.jade', {"params" : params});
